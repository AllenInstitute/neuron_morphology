...
class TestRadiusValidationFunctions(ValidationTestCase):
    ...
    def test_valid_radius_multiple_types(self):
        morphology.Morphology([test_node(id=1, type=SOMA, radius=36.0, parent_node_id=-1)
                              ...
                              , test_node(id=6, type=APICAL_DENDRITE, radius=1.0, parent_node_id=5)]
    ...
    def test_absence_of_constriction_for_dendrite_after_node_twenty_from_soma(self):
        for dendrite_type in [BASAL_DENDRITE, APICAL_DENDRITE]:
            morphology.Morphology([test_node(id=1, type=SOMA, radius=36.0, parent_node_id=-1)
                                  ...
                                  , test_node(id=20, type=dendrite_type, radius=12.0, parent_node_id=19)
                                  , test_node(id=21, type=dendrite_type, radius=1.0, parent_node_id=20)
                                  , test_node(id=22, type=dendrite_type, radius=12.0, parent_node_id=21)]
                                  , strict_validation=True)
    ...
    def test_decreasing_radius_when_going_away_from_soma_dendrite_invalid(self):
        try:
            morphology.Morphology([test_node(id=1, type=SOMA, radius=36.0, parent_node_id=-1)
                                      ...
                                      , test_node(id=5, type=APICAL_DENDRITE, radius=4.0, parent_node_id=4)]
                                      , strict_validation=True)
            self.fail("Morphology should have been rejected.")
        except InvalidMorphology, e:
